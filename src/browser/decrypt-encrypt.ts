...
// 对称加密 -- 使用前端自己的AES加密
export const encryptBySymmetric = async (
  data: string,
  keyParam?: CryptoKey
): Promise<string> => {
  let key = keyParam;
...
// 非对称加密 -- 使用后端生成的rsa公钥
export const encryptByAsymmetric = async (
  data: string,
  publicKeyParam: CryptoKey | string
): Promise<string> => {
